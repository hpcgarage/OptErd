...
C  in the file COPYING.
     ...
     +
C------------------------------------------------------------------------
C  OPERATION   : OED__NAI_SET_AB
C  MODULE      : ONE ELECTRON INTEGRALS DIRECT
C  MODULE-ID   : OED
C  SUBROUTINES : none
C  DESCRIPTION : This routine handles the logistics on how to evaluate
C                the (1|2) nuclear attraction integral batch in the
...
C                permutational symmetry of the nuclear attraction
C                integrals in (1|2):
C
C                                 (1|2) = (2|1)
C
...
C                if a zero batch of nuclear attraction integrals is
C                expected (experimental section, no zero integral
C                conditions found yet!).
C
C
         EMPTY  = .FALSE.
...
C         CASE1 = MOD (SHELLP,2) .EQ. 1
C         CASE2 = SPHERIC. AND. (SHELL1.NE.SHELL2)
C
C         IF (ATOMIC .AND. (CASE1.OR.CASE2)) THEN
C             EMPTY = .TRUE.
C             RETURN
C         END IF
C
...
C                evaluation of the [e|0] nuclear attraction batches
C                and for the HRR steps later on.
C
C
         IF (.NOT.SWAP12) THEN
             XA = X1
