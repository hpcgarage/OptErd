...
C  in the file COPYING.
         SUBROUTINE  OED__KIN_SET_IJ_PAIRS
     +
     ...
     +
C------------------------------------------------------------------------
C  OPERATION   : OED__KIN_SET_IJ_PAIRS
...
C                (if any) of kinetic primitives must be applied.
C
...
C                (s|s) kinetic integrals will be largest in magnitude
C                for all exponent combinations. One obtains the value
C                expression for the normalized (s|s) kinetic integral
C                by operating with the kinetic -1/2 DEL second
C                derivative operator on the coordinates of center B
C                in the normalized (s|s) overlap integral expression.
C                The normalized (s|s) overlap integral value expression
C                OVL is:
C
C                OVL = Sqrt(8) * (ab)**(3/4) * (a+b)**(-3/2) * rho (a,b)
C
...
C                the i-th and j-th atomic centers A and B. The
C                normalized (s|s) kinetic integral expression is then
C                obtained from:
C
C                   KIN = -1/2 * (d2/d2Bx + d2/d2By + d2/d2Bz) * OVL
C
C                and its final form is:
C
C                         KIN = OVL * (3r - 2[r*R(AB)]**2)
C
C                where r = ab/(a+b) is the exponent ratio. Note, that
C                value of KIN can also be negative, depending on the
C                separation of the two centers A and B.
C
         ...
         IF (EQUALAB) THEN
             ...
             DO I = 1,NPGTOA
                ...
                DO J = 1,I
                   ...
                   OVL = PREFACT * (AB**ZP75) * (P**NEG1P5)
                   SS = OVL * THREE * (AB/P)

                   IF (DABS (SS).GE.TOL) THEN
                       NIJ = NIJ + 1
                       RHO (NIJ) = ONE
                       PRIMA (NIJ) = I
                       PRIMB (NIJ) = J
                   ELSE
C                       WRITE (*,*) ' Skip atom kin I J = ',I,J
                   END IF
                 ...
                 ELSE
                     DO J = 1,NPGTOB
                        ...
                        DO I = 1,NPGTOA
                           ...
                           OVL = PREFACT*(AB**ZP75)*(P**NEG1P5)*RHOAB
                           SS = OVL * R * (THREE - TWO * Q)
