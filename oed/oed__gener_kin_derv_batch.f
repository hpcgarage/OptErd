...
C  in the file COPYING.
         SUBROUTINE  OED__GENER_KIN_DERV_BATCH
     +
     +                    ( IMAX,ZMAX,
     +                      NALPHA,NCOEFF,NCSUM,
     +                      NCGTO1,NCGTO2,
     ...
     +
C------------------------------------------------------------------------
C  OPERATION   : OED__GENER_KIN_DERV_BATCH
C  MODULE      : ONE ELECTRON INTEGRALS DIRECT
C  MODULE-ID   : OED
C  SUBROUTINES : OED__KIN_DERV_CSGTO
C  DESCRIPTION : Main operation that drives the calculation of a batch
C                of differentiated contracted electron kinetic
C                integrals.
C
C
C                  Input (x = 1 and 2):
