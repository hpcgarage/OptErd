...
C  in the file COPYING.
         SUBROUTINE  OED__KIN_DERV_CSGTO
     +
     +                    ( IMAX,ZMAX,
     +                      NALPHA,NCOEFF,NCSUM,
     +                      NCGTO1,NCGTO2,
     ...
     +
C------------------------------------------------------------------------
C  OPERATION   : OED__KIN_DERV_CSGTO
C  MODULE      : ONE ELECTRON INTEGRALS DIRECT
C  MODULE-ID   : OED
C  SUBROUTINES : OED__KIN_SET_DERV_AB
C                OED__KIN_SET_IJ_PAIRS
C                OED__KIN_SET_DERV_SEQUENCE
C                OED__KIN_DERV_DEF_BLOCKS
C                OED__KIN_PREPARE_CTR
C                OED__KIN_DERV_PCGTO_BLOCK
...
C                contracted kinetic integrals on up to two different
...
C                                    derivative kinetic integrals over
...
C                derivative kinetic [A|B] integrals will be essential
...
C                Since all derivatives of the kinetic integrals on
...
C
         ...
         CALL  OED__KIN_SET_DERV_SEQUENCE
     +
     +              ( NDERX,NDERY,NDERZ,
     +                DERAX,DERAY,DERAZ,
     +                DERBX,DERBY,DERBZ,
...
C                kinetic integrals in blocks over ij pairs and add to
C                final contracted (a|b) derivative kinetic integrals.
C                The keyword REORDER indicates, if the primitive [a|b]
C                derivative kinetic integrals need to be transposed
...
C
         ...
         DO 1000 NIJBEG = 1,NIJ,NIJBLK
            ...
            CALL  OED__KIN_DERV_PCGTO_BLOCK
     +
     +                 ( NBATCH,
     +                   NKIN1D,
     +                   NOVL1DX,NOVL1DY,NOVL1DZ,
     ...
     +                   ZCORE (ZRHOAB),
     +                   ZCORE (ZEA),ZCORE (ZEB),
     +                   ZCORE (ZE2A),ZCORE (ZE2B),ZCORE (ZE2AB),
     ...
     +
     +
C            WRITE (*,*) ' Finished ab kinetic pcgto derv block '
